/**
 * configuration.js - Copyright(C) 2021 Donald Zou [https://github.com/AmirOo0]
...
$add_peer.addEventListener("click", function () {
    ...
    if ($bulk_add.prop("checked")) {
        ...
        let $new_peer_volume = $("#new_peer_volume");
        let $new_peer_expire = $("#new_peer_expire");
        let $new_add_MTU = $("#new_add_MTU");
        let $new_add_keep_alive = $("#new_add_keep_alive");
        let $enable_preshare_key = $("#enable_preshare_key");
        $add_peer.setAttribute("disabled", "disabled");
        ...
        if ($allowed_ips.val() !== "" && $public_key.val() !== "" && $new_add_DNS.val() !== "" && $new_add_endpoint_allowed_ip.val() !== "") {
            let conf = $add_peer.getAttribute('conf_id');
            let data_list = [$private_key, $allowed_ips, $new_add_name, $new_peer_volume, $new_peer_expire, $new_add_DNS, $new_add_endpoint_allowed_ip, $new_add_MTU, $new_add_keep_alive];
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    "volume": $new_peer_volume.val(),
                    "expires_at": +new Date($new_peer_expire.val()) / 1000,
                    "DNS": $new_add_DNS.val(),
                    "endpoint_allowed_ip": $new_add_endpoint_allowed_ip.val(),
                    "MTU": $new_add_MTU.val(),
                    "keep_alive": $new_add_keep_alive.val(),
...
$body.on("click", ".btn-setting-peer", function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#setting_modal #peer_expire_textbox").val(formatDate(new Date(response.expires_at)));
            $("#setting_modal #peer_volume_textbox").val((parseFloat(response.volume) / 1024 ** 3).toFixed(4));
...
$("#save_peer_setting").on("click", function () {
    ...
    let $peer_volume_textbox = $("#peer_volume_textbox");
    let $peer_expire_textbox = $("#peer_expire_textbox");
    ...
    if ($peer_DNS_textbox.val() !== "" &&
        ...
        let data_list = [$peer_name_textbox, $peer_volume_textbox, $peer_expire_textbox, $peer_DNS_textbox, $peer_private_key_textbox, $peer_preshared_key_textbox, $peer_allowed_ip_textbox, $peer_endpoint_allowed_ips, $peer_mtu, $peer_keep_alive];
        data_list.forEach((ele) => ele.attr("disabled", "disabled"));
        $.ajax({
            method: "POST",
            ...
            data: JSON.stringify({
                ...
                volume: $peer_volume_textbox.val(),
                expires_at: +new Date($peer_expire_textbox.val()) / 1000,
