...
def get_latest_handshake(config_name):
    ...
    for peer in peers:
        ...
        if handshake:
            ...
            if handshake['time'] > 0:
                query = f"update {config_name} set latest_handshake = ?, status = ?, is_connected = ?, expires_at = ? where id=?"
                bindings = (str(diff).split('.', maxsplit=1)[0], status, is_connected, expires_at, key)
